...
function guarded(req, res) {
  const credentionals = getCredentionals(req.headers?.cookie);// замінити тут
  if(!credentionals) {
    res.writeHead(302, {'Location': '/register'})// замінити тут
    return res.end();
  }
  if(req.method === 'GET') {
    switch(req.url) {
...
function getCredentionals(c = '') { // замінити тут
  const cookies = cookie.parse(c); // замінити тут
